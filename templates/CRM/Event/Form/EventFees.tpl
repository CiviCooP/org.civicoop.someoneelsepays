...
{*Someone Else Pays information (extension org.civicoop.someoneelsepays *}
<br />
<div id="sep_eventfees_add_block" class="crm-accordion-wrapper">
  <div class="crm-accordion-header">{ts}Someone Else Pays{/ts}</div>
  <div class="crm-accordion-body">
    <fieldset id="sep_eventfees_fieldset">
      <table class="form-layout">
        <tbody>
        <tr class="crm-eventfees-form-block-sep_flag">
          <td class="label">{$form.sep_flag.label}</td>
          <td>{$form.sep_flag.html}</td>
        </tr>
        <tr class="crm-eventfees-form-block-sep_payer_id">
          <td class="label">{$form.sep_payer_id.label}</td>
          <td>{$form.sep_payer_id.html}</td>
        </tr>
        </tbody>
      </table>
    </fieldset>
...
<script>
  ...
  {/if}
  {literal}
    cj('#CIVICRM_QFID_1_sep_flag').click(function() {
      if (this.value === '1') {
        cj('.crm-eventfees-form-block-sep_payer_id').show();
      }
    });
    cj('#CIVICRM_QFID_0_sep_flag').click(function() {
      if (this.value === '0') {
        cj('.crm-eventfees-form-block-sep_payer_id').hide();
      }
    });
